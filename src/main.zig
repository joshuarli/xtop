...
pub fn main() !void {
    ...
    var proc_store = std.AutoHashMap(ProcessKey, Process).init(allocator);
    defer { store.freeStore(&proc_store, allocator); proc_store.deinit(); }
    ...
    while (!shutdown_flag.load(.acquire)) {
        ...
        if (resize_flag.swap(false, .acq_rel)) mem.readMemInfo(&mem_state);

        const pressed = key_event.swap(0, .acq_rel);
        switch (pressed) {
            'c' => sort_key = .cpu, 'm' => sort_key = .mem,
            'q' => shutdown_flag.store(true, .release),
            else => {},
        }
        if (shutdown_flag.load(.acquire)) break;

        try runTick(allocator, io, tick, &syscpu, &mem_state, &net_state, &proc_store, &proc_list, sort_key, null);
...
fn runTick(
    ...
    mem_state: *MemState,
    net_state: *NetState,
    proc_store: *std.AutoHashMap(ProcessKey, Process),
    proc_list: *std.ArrayList(*Process),
    sort_key: SortKey,
    capture_dir: ?[]const u8,
) !void {
    var pid_buf: [16384]Pid = undefined;
    const pids = scan.scanProc(&pid_buf) catch |err| {
        std.debug.print("scan error: {}\n", .{err}); return;
    };

    cpu.readCpuStat(syscpu) catch {};

    for (syscpu.cores[0..syscpu.num_cores], 0..) |core, ci| {
        const prev = syscpu.prev_cores[ci];
        const td = core.total() -| prev.total();
        if (td > 0) {
            const ad = core.active() -| prev.active();
            syscpu.core_history[ci].append(.{ .cpu_pct = @as(f32, @floatFromInt(ad)) / @as(f32, @floatFromInt(td)) * 100.0, .ts_ms = @intCast(tick) });
        }
    }
    ...
    const mp: f32 = if (mem_state.total_kb > 0)
        @as(f32, @floatFromInt(mem_state.total_kb -| mem_state.avail_kb)) / @as(f32, @floatFromInt(mem_state.total_kb)) * 100.0 else 0;
    ...
    for (pids) |pid| { futures.append(allocator, Io.async(io, proc.readProcData, .{pid})) catch break; }
    ...
    for (futures.items) |*future| {
        ...
        if (proc_store.getPtr(key)) |existing| {
            ...
            var old_iter = proc_store.iterator();
            while (old_iter.next()) |entry| {
                if (entry.key_ptr.pid == result.pid and entry.key_ptr.starttime != result.starttime) {
                    if (entry.value_ptr.ring) |ring| allocator.destroy(ring);
                    _ = proc_store.remove(entry.key_ptr.*);
                    break;
                }
            }
            ...
            proc_store.put(key, new_proc) catch continue;
        }
    }
    ...
    while (iter.next()) |entry| { proc_list.append(allocator, entry.value_ptr) catch break; }
    switch (sort_key) {
    ...
    if (capture_dir) |dir| {
        ...
        render.render(syscpu, mem_state, net_state, top, sort_key) catch {};
    }
}
...
fn inputThreadFn() void {
    ...
    while (!shutdown_flag.load(.acquire)) {
        ...
        if (n == 1) {
            ...
            if (buf[0] == 'q') { shutdown_flag.store(true, .release); return; }
        }
    }
}
...
fn sigwinchHandler(_: std.posix.SIG) callconv(.c) void { resize_flag.store(true, .monotonic); }
fn sigtermHandler(_: std.posix.SIG) callconv(.c) void { shutdown_flag.store(true, .release); }
...
fn runCapture(allocator: std.mem.Allocator, cap_dir: []const u8) !void {
    ...
    while (tick < max_ticks and !shutdown_flag.load(.acquire)) {
        ...
        try runTick(allocator, io, tick, &syscpu, &mem_state, &net_state, &proc_store, &proc_list, .cpu, cap_dir);
...
fn sleepMs(ms: u64) void { sleepNs(@as(i96, @intCast(ms)) * std.time.ns_per_ms); }
